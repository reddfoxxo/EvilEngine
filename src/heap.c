...
void* heap_alloc(heap_t* heap, size_t size, size_t alignment)
{
	...
	int n = debug_backtrace(address, 16);
...
static void heap_walker(void* ptr, size_t size, int used, void* user)
{
	if (used)
	{
		...
		// https://stackoverflow.com/questions/5693192/win32-backtrace-from-c-code/5699483#5699483
...
void heap_destroy(heap_t* heap)
{
	...
	while (arena)
	{
		...
		tlsf_walk_pool(arena->pool, heap_walker, heap);
